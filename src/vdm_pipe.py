...
# from src.ic_light_normal import Relighter2

import torch.fft as fft
from einops import rearrange
from diffusers.utils import export_to_gif
from src.animatediff_eul import eul_step
from src.animatediff_ddim import ddim_step
import math
...
class AnimateDiffVideoToVideoPipeline(
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            if batch_size > init_latents.shape[0] and batch_size % init_latents.shape[0] == 0:
                ...
                init_latents = torch.cat([init_latents], dim=0) 
            noise = randn_tensor(init_latents.shape, generator=generator, device=device, dtype=dtype)#torch.Size([1, 16, 4, 64, 64])
    ...
    def __call__(
        ...
        if latents is None:
            video = self.video_processor.preprocess_video(video, height=height, width=width)#video.shape: torch.Size([1, 3, 16, 512, 512])
            ...
            print("normal_video.shape:",normal_video.shape)
            normal_video = normal_video.permute(0, 2, 1, 3, 4)
            normal_video = normal_video.to(device=device, dtype=prompt_embeds.dtype) ## torch.Size([1, 16, 3, 512, 512])
            normal_target = rearrange(normal_video, "1 f c h w -> f c h w")
        ...
        def inject_light_map_noise(x_T_total, bg_source,R, device, total_frames):
                    ...
                    num,batch_size, channels, total_frames, image_height, image_width = x_T_total.shape
                    max_pix = 255
                    min_pix = 0
            ...
            with self.progress_bar(total=self._num_timesteps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    if lbd>0.15:
                        ...
                        ## consistent target
                        consist_target = self.decode_latents(pred_x0_latent.to(dtype=self.vae.dtype))
                        ...
                        consist_target= consist_target.to(dtype=self.vae.dtype)
                        normal_target= normal_target.to(dtype=self.vae.dtype)


                        relight_target = relighter(consist_target,normal_target,lbd)
                        ...
                        output = eul_step(self.scheduler, noise_pred, t, latents, fusion_latent, self, **extra_step_kwargs, return_dict=False) 
